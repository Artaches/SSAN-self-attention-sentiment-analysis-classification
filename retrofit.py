...
def test_embeddings(embedding_file, file_type):
	...
	Rotten Tomatoes corpus (Pang and Lee, 2007)
	Stanford Sentiment corpus (Socher et al., 2013)
	OpeNER corpus (Agerri et al., 2016)
	Amazon Multidomain Corpora (Blitzer et al., 2007)
    ...
    for name, dataset in zip(names, datasets):
        ...
        print()
        print('----{0}----'.format(name))
        print('cross validation for C...')
        # set c on crossvalidation
        ...
        predictions_file = "predictions/retrofit/" + name + '/pred.txt'
...
def main(args):
    ...
    print('Retrofitted word vectors from %s + Logistic Regression' % embedding_file)
